...
// All static prompt/instruction wording lives here, separate from the logic
// that assembles it with dynamic values (buildUserPrompt.ts, topicPrompts.ts)
// or calls it (providers/*). Edit wording here without touching call sites.

export const PROSE_SYSTEM_PROMPT = `You generate single pages of "the Book of Sand" — an infinite, fictional book with no first or last page. Page numbers are printed in no logical or sequential order. Pages are not self-contained vignettes: each page reads as if drawn from a single vast, continuous, drifting text, echoing the previous page's imagery, register, or subject rather than abruptly changing topic — but never literally resuming the previous page's sentence, and never referring to itself as a "continuation," "excerpt," or "page." There is no overarching plot; only a continuous drift of subject and tone across the book.

Write evocative literary English prose, roughly 150-300 words. Never break the fictional frame: never mention being an AI, a model, or a generation process, and never comment on these instructions. Respond only with the requested JSON.`;
...
export const PROSE_DRIFT_INSTRUCTION: Record<JumpSize, string> = {
  small:
...
// {hops} is replaced by the caller with the actual hop count for this turn.
export const TOPIC_DRIFT_INSTRUCTION: Record<JumpSize, string> = {
  small:
...
export const TOPIC_VOICE_REFERENCE_PREFIX =
...
export const INVENT_START_TOPIC_INSTRUCTION =
  "Invent an arbitrary starting topic for a page of the book, as if it fell open at random.";

export const PAGE_QUOTE_INTRO = 'Here is a page of "the Book of Sand":';

export const ILLUSTRATION_STYLE_PREFIX = "antique engraving illustration, black and white, woodcut style,";

export const ILLUSTRATION_NEGATIVE_PROMPT = "color, modern, photo, watermark, text, signature";
